...
class Protein:
    ...
    def read_in_3Dstructure(self,pdbfile,chains=None):
        ...
        (?P<user>[0-9, ,\-,\.]{6})              # user field
        (?P<beta>[0-9, ,\-,\.]{6})              # beta field
        (?P<elementType>[A-Z,0-9, ]*)           # Element Type - NOT PRESENT
        (?P<charge>[A-Z,0-9,\-,\+, ]*)          # Charge - NOT PRESENT
        """
        mask2 = re.compile(shortLinePattern, re.VERBOSE)
        ...
        for line in liste:
            ...
            #deal with dyes that babel can't process
            #fairly terrible hack
            if "Cd" in line:
                continue
            if "Ne" in line:
                continue
    ...
    def min_Distance_to_Set(self, centre=None,AtomSet=None):
        ...
        for Setatom in AtomSet:
            ...
            if d < min_distance:
                min_distance = d
                min_atom = Setatom
        return min_distance,min_atom

    def distanceBetweenTwoSets(self, setA, setB):
        min_distance = 777
        for atom in setA:
            d = self.min_Distance_to_Set(centre=atom,AtomSet=setB)
            if d < min_distance:
                #print d
                min_distance = d
        return min_distance
    
    def clashBetweenTwoSets(self, setA, setB):
        #define a clash as a distance lt 2 Angstrom
        min_distance = 777
        for atom in setA:
            for secondatom in setB:
                d = secondatom.distanceTo(atom)
                if d < min_distance:
                    min_distance = d
                    if min_distance < 2.0:
                        return True
